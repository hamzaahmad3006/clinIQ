-- ClinIQ Supabase Schema + Seed Data (USA Names)
...
DELETE FROM break_glass_events;
DELETE FROM audit_logs;
DELETE FROM treatment_relationships;
DELETE FROM clinical_flags;
DELETE FROM overnight_events;
...
INSERT INTO patients (id, name, initials, nhs_number, date_of_birth, age, gender, avatar_color, avatar_text_color, consent_status, organisation_id) VALUES
  ('j-wilson', 'J. Wilson', 'JW', '482-192-1121', '1958-03-12', 68, 'Male', '#D6E8FA', '#2E7DD1', 'active', 'nhs-trust-001'),
  ('m-thompson', 'M. Thompson', 'MT', '591-231-0081', '1975-08-22', 51, 'Male', '#FEF3C7', '#B45309', 'active', 'nhs-trust-001'),
  ('r-johnson', 'R. Johnson', 'RJ', '109-332-9456', '1981-04-15', 45, 'Female', '#FEE2E2', '#D92B2B', 'active', 'nhs-trust-001'),
  ('d-brown', 'D. Brown', 'DB', '739-441-8823', '1944-11-03', 82, 'Female', '#DBEAFE', '#1E40AF', 'active', 'nhs-trust-001'),
  ('w-davis', 'W. Davis', 'WD', '284-109-5567', '1965-06-30', 61, 'Male', '#F3E8FF', '#7C3AED', 'active', 'nhs-trust-001'),
  ('s-miller', 'S. Miller', 'SM', '601-778-2340', '1997-02-14', 29, 'Female', '#D1FAE5', '#059669', 'active', 'nhs-trust-001')
...
('j-wilson', 'James Wilson', '482 192 1121', '12 March 1958', 68, 'Male', 'Cardiology', 'Dr. Henderson',
...
('m-thompson', 'Michael Thompson', '591 231 0081', '22 August 1975', 51, 'Male', 'General Medicine', 'Dr. Andrews',
  '[]',
  ...
  '[]'),
('r-johnson', 'Rebecca Johnson', '109 332 9456', '15 April 1981', 45, 'Female', 'Emergency Medicine', 'Dr. Andrews',
  ...
  '[{"type": "critical", "title": "Medication Discrepancy", "description": "Metformin dose mismatch between PCP record and discharge summary. Reconciliation required."}]'),
('d-brown', 'Dorothy Brown', '739 441 8823', '03 November 1944', 82, 'Female', 'General Medicine', 'Dr. Henderson',
  '[]',
  ...
  '[]'),
('w-davis', 'William Davis', '284 109 5567', '30 June 1965', 61, 'Male', 'General Medicine', 'Dr. Henderson',
...
('s-miller', 'Sarah Miller', '601 778 2340', '14 February 1997', 29, 'Female', 'General Surgery', 'Dr. Henderson',
  '[]',
...
INSERT INTO encounters (id, patient_id, patient_name, patient_initials, nhs_number, encounter_type, department, status, status_label, status_color, border_color, brief_status, brief_percentage, flag_count, avatar_color, avatar_text_color) VALUES
  ('enc-001', 'j-wilson', 'J. Wilson', 'JW', '482-192-1121', 'inpatient', 'Cardiology', 'admitted', 'ADMITTED', 'bg-low-bg text-low-severity border border-outline-variant', 'border-green-500', 'ready', 100, 1, '#D6E8FA', '#2E7DD1'),
  ('enc-002', 'm-thompson', 'M. Thompson', 'MT', '591-231-0081', 'inpatient', 'General Medicine', 'labs_pending', 'LABS PENDING', 'bg-high-bg text-high-severity border border-high-severity', 'border-yellow-500', 'syncing', 60, 0, '#FEF3C7', '#B45309'),
  ('enc-003', 'r-johnson', 'R. Johnson', 'RJ', '109-332-9456', 'emergency', 'Emergency Medicine', 'urgent', 'URGENT', 'bg-critical-bg text-critical border border-critical', 'border-critical', 'ready', 100, 2, '#FEE2E2', '#D92B2B')
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO ward_beds (bed, patient_id, patient_name, age, gender, brief_status, overnight_changes, overnight_detail, flag_count, flag_severity) VALUES
  ('4B1', 'd-brown', 'D. Brown', 82, 'Female', 'ready', '["2 new obs"]', 'NEWS 4 → 6', 1, 'critical'),
  ('4B2', 'r-johnson', 'R. Johnson', 45, 'Female', 'ready', '["New labs ↑"]', '', 3, 'high'),
  ('4B3', 'w-davis', 'W. Davis', 61, 'Male', 'ready', '["Creatinine 201 ↑"]', 'AKI Stage 1 Detected', 2, 'critical'),
  ('4B4', 's-miller', 'S. Miller', 29, 'Female', 'ready', '[]', 'No significant change', 0, 'none')
ON CONFLICT (bed) DO NOTHING;

-- Overnight Events
INSERT INTO overnight_events (patient_name, bed, time, severity, title, description) VALUES
  ('W. Davis', '4B3', '04:12', 'critical', 'CRITICAL LAB ALERT', 'Creatinine increased from 115 to 201 µmol/L (AKI Stage 1). Potassium 5.4. No change in urine output noted in nursing notes.'),
  ('D. Brown', '4B1', '03:45', 'high', 'OBSERVATION TREND', 'NEWS score escalated from 4 to 6. Tachycardia (112) and slight drop in O2 sats (93% on air). RMO reviewed and increased supplemental O2.'),
  ('S. Miller', '4B4', '01:20', 'low', 'CLINICAL NOTE', 'Patient slept well. PRN analgesic administered for minor abdominal pain at 00:30. Effectively settled.')
ON CONFLICT DO NOTHING;

-- Clinical Flags
INSERT INTO clinical_flags (id, patient_id, patient_name, flag_type, severity, description, source, created_at) VALUES
  ('flag-001', 'j-wilson', 'J. Wilson', 'allergy_conflict', 'critical', 'Prescribed Penicillin (V-Cil-K) despite recorded allergy to Penicillins. Brief generation flagged clinical risk.', 'EPIC / Medication-Admin-Log v1.4', '2026-06-03T07:45:00Z'),
  ('flag-002', 'r-johnson', 'R. Johnson', 'med_discrepancy', 'high', 'Metformin dose mismatch between PCP record (1000mg BD) and discharge summary (500mg BD). Source reconciliation required.', 'GP Connect / SystmOne v4.2', '2026-06-03T06:30:00Z'),
  ('flag-003', 'r-johnson', 'R. Johnson', 'contraindication', 'critical', 'Active CKD Stage 3 with eGFR 48. Metformin requires dose review at eGFR < 45. Approaching threshold.', 'PathologyLIS / Renal Panel', '2026-06-03T04:15:00Z')
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO treatment_relationships (patient_id, clinician_id, clinician_name, relationship_type, valid_from, valid_until, status) VALUES
  ('j-wilson', 'clin-henderson-001', 'Dr. Henderson', 'ward_doctor', '2026-06-01T00:00:00Z', NULL, 'active'),
  ('d-brown', 'clin-henderson-001', 'Dr. Henderson', 'ward_doctor', '2026-06-01T00:00:00Z', NULL, 'active'),
  ('w-davis', 'clin-henderson-001', 'Dr. Henderson', 'ward_doctor', '2026-06-01T00:00:00Z', NULL, 'active'),
  ('s-miller', 'clin-henderson-001', 'Dr. Henderson', 'ward_doctor', '2026-06-01T00:00:00Z', NULL, 'active'),
  ('m-thompson', 'clin-andrews-001', 'Dr. Andrews', 'ward_doctor', '2026-06-01T00:00:00Z', NULL, 'active'),
  ('r-johnson', 'clin-andrews-001', 'Dr. Andrews', 'emergency', '2026-06-01T00:00:00Z', NULL, 'active'),
  ('r-johnson', 'clin-henderson-001', 'Dr. Henderson', 'referral', '2026-06-03T00:00:00Z', '2026-06-10T00:00:00Z', 'active')
ON CONFLICT DO NOTHING;
...
INSERT INTO audit_logs (actor_id, actor_name, actor_role, action, resource_type, resource_id, patient_id, access_result, sensitivity_tier, timestamp) VALUES
  ('clin-andrews-001', 'Dr. Andrews', 'emergency_physician', 'auth.login.success', 'session', 'sess-001', NULL, 'granted', NULL, '2026-06-03T07:30:00Z')
ON CONFLICT DO NOTHING;
